...
Notation (used once here, never redefined): k_rate is the game strikeout rate (K/PA); TBF is batters faced (projected, pregame); E[K] is expected strikeouts; P(K >= L) is the line probability; p_model is our taken-side probability; p_market is the devigged market probability; edge = p_model − p_market; CLV_pp is closing line value in probability points; xCLV/xROI are model-expected close-beat and mean taken edge (belief, not results); LCB is the 95% lower confidence bound on ROI; WR is win rate; Sharpe/Sortino scale return by (downside) volatility; drawdown/Calmar measure peak-to-trough pain and its ratio; beat rate is the fraction of tickets beating the close; profit factor is gross profit over gross loss.
...
**Priority ladder (which metric governs what).** Model metrics (MAE, Brier skill) govern the *model* -- they never promote a bet. Betting metrics govern *selection*: LCB first (worst-plausible return), CLV second (execution quality), realized ROI third (scoreboard). Quant metrics govern *survival*: Sharpe/Sortino for risk-adjusted comparison, drawdown/Calmar for position sizing and stop rules, concentration for single-cell risk. Missing from this report and tracked going forward: profit factor (1.15 juiced -- thin margin, every basis point is selection), average taken price (longshot-leaning book), turnover (stake-days per season). Beat rate is reported beside every CLV mean because a mean without a hit rate hides skew.
...
**Ahead despite worse calibration.** The project is ahead of the books on every money metric (ROI +7.3% juiced, +15.5% champion-select, Sharpe 2.79 on the 2026 judge) while trailing them on every probability metric (Brier skill -0.0043, ECE 0.021 vs 0.009). There is no contradiction: books are calibrated across everything they must price; the desk is profitable on the subset it chooses to bet. Calibration is a property of the full distribution, profit a property of the selected tail -- and selection, not prediction, is the entire edge.
...
**Simulated PnL/ROI (fair-price harness, upper bound).** The retired fair-price harness champion lane (floor 0.08, veto off, snap-best) printed 2025 PnL +$146,216 (ROI +51%, n=5,694) and 2026 PnL +$48,052 (ROI +32.9%, n=2,920) at vig-free fair prices with CLV +14.7pp/+8.5pp and xROI 0.21/0.17. These are explicitly NOT attainable figures: fair prices remove the vig, open-mixed snaps include unbettable stale quotes, and fills are unmodeled. They bound the upside and calibrate the haircut between model belief (+51%) and executable reality (+7.3% juiced, +15.5% champion-select). No policy was ever promoted from this lane.

**Consensus books (transparency).** Devigged-median consensus pools nine US-region books -- DraftKings, FanDuel, BetMGM, Bovada, BetOnline, Fanatics, BetRivers, William Hill US, mybookieag -- per-event two-way pairs (2+ books required), median across books. BetRivers contributes ~3x the rows of a typical book (alt-line bleed, QA-flagged; median is robust to it). No Pinnacle (absent for MLB props in-region): Kalshi is the sharp reference, DK/FD devigged close the book benchmark. CLV flavors: same-book fillable +0.11pp/42% (headline), consensus +0.26pp/56% (reference), cross-book +0.61pp/59% (soft-read, never headlined).
