...
| -------------------------------------------- | --------------- | --------------- | -------------- | ----------------------- |
| None (full model)                            | 0.0789 / 0.0838 | 0.0996 / 0.1057 | 0.101 / −0.021 | --                      |
| - Park factors                               | 0.0792 / 0.0840 | —               | —              | +0.00026 / +0.00018     |
| - Opponent lineup features                   | 0.0815 / 0.0861 | —               | —              | **+0.00254 / +0.00231** |
| - Season-to-date (keep rolling only)         | 0.0791 / 0.0800 | —               | —              | +0.00016 / −0.00380     |
| - Rolling windows (keep season-to-date only) | 0.0802 / 0.0778 | —               | —              | **+0.00125 / −0.00598** |
| - Pitch-arsenal physics (velo/spin/movement) | 0.0794 / 0.0812 | —               | —              | +0.00052 / −0.00267     |


Values are **LightGBM / Ridge** mean outer-fold metrics from the 2026-07-27
leave-family-out screen (`docs/step3_leave_family_out_findings.md`,
...
`16.432`, and two values above 10 (`xwOBA_P5` and `xFIP_P5`). Step 1 closed by
adopting this as the Ridge research registry with one amendment: drop
`xFIP_P5` (73 features; residual VIF only on `xwOBA_P5`). LightGBM keeps the
...
Step 4 closed for the LightGBM backbone (`docs/step4_window_decisions.md`):
...
- apply Step 4 LGBM freeze proposals at registry freeze (`docs/step4_window_decisions.md`):
...
(`docs/phase_d_population_findings.md`). Live assembly is optional next;
pristine v1 still needs pregame role labels.

1. ~~Complete a feature dictionary with missingness rates and correlation
  clusters by family.~~ **Done** — `docs/step1_feature_dict_vif_findings.md`.
...
3. ~~Run grouped ablations across every major family.~~ **Done** —
  `docs/step3_leave_family_out_findings.md`, `docs/step8_feature_keep_drop_findings.md`.
4. ~~Select windows per statistic.~~ **Done** — `docs/step4_window_decisions.md`,
  `docs/step9_metric_window_findings.md`, Step 9c/10 P1 lock.
5. ~~Compare unweighted vs PA-weighted / binomial / beta-binomial.~~ **Done** —
  keep unweighted LightGBM; see `docs/step5_*_findings.md`.
...
7. ~~Freeze compact feature registry.~~ **Done** — **180**-feature production
  (`docs/step10_p1_registry_freeze.md`; `step7_185` retained for bake-offs).
8. ~~Estimator / stack quality (Phase 11.A–C).~~ **Done** — confirmatory; keep
  baseline LGBM defaults + Ridge joblib (`docs/phase11_model_quality_gates.md`).
9. ~~Phase D population audit + interim policy.~~ **Done** — ~3.5% of first
  pitchers excluded by `PA≥9`; metrics remain conditional.
10. Protected evaluation: historical `2025` is **not** pristine. Next honest
   final test = **future post-freeze games** after pregame role labels.

Detailed audit: `docs/statistical_audit_and_sequencing_report.md`. Rest /
bullpen / TBF: `docs/workload_rest_bullpen_feature_plan.md`.

## 13. Strikeout-count probability modeling

**Status (2026-07-28):** count layer + **walk-forward stack** + **calibration**
shipped under Phase 11 (`docs/phase11_model_quality_gates.md`,
...
- Refresh Savant → Level 1–2 through yesterday (unlocks true 2026 live scores);
- Pregame role labels for pristine v1 (Phase D completion);
- negative-binomial challenger with log-TBF offset (not built);
- mixing the count distribution over a **TBF distribution** (point TBF only today);
- market de-vig / fractional Kelly (product layer).

Live assembly v1 is wired (`docs/live_assembly_plan.md`): historical score on
2025-09-20 proven; live scrape needs fresh rolling data.
