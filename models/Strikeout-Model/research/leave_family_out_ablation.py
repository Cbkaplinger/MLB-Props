...
def _family_map(features: list[str]) -> dict[str, list[str]]:
    ...
    for feature in features:
        family = mapping.get(feature)
        if family is None:
            # Backstop for newer registries when dictionary lag exists.
            if feature.startswith("opp_lineup_"):
                family = "lineup"
            elif "_P" in feature or feature.endswith("_std"):
                family = "rolling_misc"
            else:
                family = "other"
        families.setdefault(str(family), []).append(feature)
    return families
