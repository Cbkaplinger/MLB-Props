"""Nested-CV Optuna tuning for production_final58_consensus.

Objective:
- Minimize expected_K MAE (primary)
...
Example:
    python models/Strikeout-Model/research/tune_lightgbm_final58_optuna.py --trials 40
"""
...
def main() -> None:
    ...
    output_dir = config.OUTPUT_DIR / "model_quality" / "final58_optuna_nested"
    ...
    for outer_name, nested in folds.items():
        ...
        def objective(trial):
            ...
            # Soft penalties enforce guardrails without hard-pruning all candidates.
            penalty = 0.0
        ...
        mae_ek_minus = _expected_k_mae(
            nested.outer.validation["K"], k_hat_outer, tbf_hat_outer * 0.97
        )
        mae_ek_plus = _expected_k_mae(
            nested.outer.validation["K"], k_hat_outer, tbf_hat_outer * 1.03
        )
    ...
    payload = {
        ...
        "feature_set": "production_final58_consensus",
